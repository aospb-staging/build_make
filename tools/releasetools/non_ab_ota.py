...
""" % bcb_dev)
  ...
  build_tag = target_info.GetBuildProp("ro.build.tags")
  release_type = target_info.GetBuildProp("ro.custom.releasetype")
  ...
  script.Print(" Build Tag: %s"%(build_tag));
  script.Print(" Security patch: %s"%(security_patch));
  script.Print(" Device: %s"%(device));
  script.Print(" Release Type: %s"%(release_type));
  script.Print("------------------------------------------");
